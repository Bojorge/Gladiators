...
void *controlComunicacion(void*){
    ...
    for (int m = 0; m <= trrs.size() - 1; m++) {
        ...
        cout << "agregando a " << torreDes.getNombre() << endl;
    ...
    while(true){
        ...
        if(EnviarGladiadores==true){
            string paquete="$";
            for(int i=0;gladiadoresmuertos.size()-1;i++){
                string gldrs=serial.serializadorGladiador(gladiadoresmuertos.getbyposicion(i));

                paquete=paquete+"/"+";"+"/"+gldrs;
            }
            gladiadoresmuertos.del_all(); //se eliminan la lista de los gladiadores muertos
...
void *mensajes(void*) {
    ...
    g1.setResistencia(11);
    g2.setResistencia(12);
    g3.setResistencia(10);
    g4.setResistencia(10);
